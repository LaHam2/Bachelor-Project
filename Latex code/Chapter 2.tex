...
defined by
\[
  \pi_B(A) = \pi(A \cap B)/\pi(B); \qquad \pi_B(B) = 1,
\]
and
\begin{align}
  |\pi_B - \pi|_1
    &= \int_B |\pi_B(dP) - \pi(dP)| + \pi(\bar B)
     = \int_B \frac{1 - \pi(B)}{\pi(B)}\,\pi(dP) + \pi(\bar B) \notag\\
...
(see Proposition~2.1), it suffices to check that
\[
  |P_{\tilde\pi_{l,n}} - P_{\pi_{l,n}}|_1 \to 0, \qquad l = 0, 1.
\]
The last relation follows from \textcolor{red}{Add eq 2.32}:
\[
