...
The goal of this section will be two prove the following lower bound on the sample complexity of CAT testing, i.e. the minimum size of the sample for which there exists a CAT test that tests the two hypotheses against each other with probability error less than $\delta$. 
This result can be summarized in the following theorem. 
\begin{theorem}\label{samplecomplexitythm}\citet{gerberclas}
    Given a known distribution $\P_0$ following a Gaussian sequence model with mean vector in the Sobolev ellipsoid $\mathcal{E}(s,C)$, consider the following goodness-of-fit problem : 
    \begin{equation*}
        \H_0 : \P_X = \P_0 \ \text{ versus } \ \H_1 : TV(\P_X,\P_0) \geq \epsilon
    \end{equation*}
    Where $P_X$ is the distribution of our sample $X$. Let $n(\epsilon, \delta, s)$ be the smallest number such that for all $n\geq n(\epsilon,\delta,s)$ there exists a function 
    $\psi : \mx^n \rightarrow \{0,1\}$ which given our sample $X$ as input, tests between $\H_0$ and $\H_1$ with error probability at most $\delta$ for every $\P_X,\P_0 \in \mathcal{E}(s,C)$.
    Then : 
    \begin{equation*}
        n(\epsilon, \delta,s) \gtrsim \frac{\sqrt{\log(\frac{1}{\delta})}}{\epsilon^{\frac{4s+1}{2s}}} + \frac{\log(\frac{1}{\delta})}{\epsilon^2}
    \end{equation*}
\end{theorem}
...
\begin{proof}[Proof of \Cref{prop1}]
    ...
    \end{equation*}
    However, this inequality follows from \Cref{sixseven}. In fact : \begin{equation*}
        \textcolor{red}{Add more details}
    \end{equation*}
...
\end{lemma}
\begin{proof}[Proof of \Cref{LipschitzConc}] 
    \textcolor{red}{Add proof, log-Sobolev inequality}
\end{proof}
...
\begin{proof}[Proof of \Cref{lemma13gerber}]
    ...
    \begin{align*}
        ...
        & \leq \frac{C}{\sqrt{8 \pi e}}
    \end{align*}
    By Lipschitz concentration of Gaussian random variable (\Cref{LipschitzConc}), one can conclude that $g - \E[g|U]$ is sub-Gaussian with variance proxy $\frac{C^2}{4\pi en}$. Now, we study the concentration of $\E[g|U]$. First, note that : \begin{equation*}
    ...
    Where $W \coloneqq \scal{\theta^X-\theta^Y}{\frac{U}{\norm{U}}}$. To simplify notations, let $\varphi \coloneqq \Phi\Bigl(\frac{\cdot}{\sqrt{4+\frac{2}{n}}}\Bigr)$, by Lipschitness of $\Phi$, we obtain that for $t \geq 0$: \begin{align*}
        \P\Bigl( \Big|\varphi(W)-\E\varphi(W) \Big| \geq t\Bigr) &\leq \P\Bigl(\Bigr) \textcolor{red}{Finish the proof}
    \end{align*}
...
\begin{proof} [Proof of \Cref{rubignolle}] We will show that each one of the two inequalities hold true using Jensen inequality. 
 \begin{itemize}
    \item Equivalently, we will show that : \begin{equation*}
        \int \log\Bigl(\frac{d\mu}{d\nu}\Bigr)d\mu = KL(\mu\Vert\nu) \leq \log(1+\chi^2(\mu\Vert\nu)) = \log\Bigl(\int \frac{d\mu}{d\nu}d\mu\Bigr)
    \end{equation*}
    Which is a direct consequence of Jensen inequality (since $\log$ is concave)
    \item Instead of the inequality above, one will show that : \begin{equation*}
...
\begin{prop}\label{param}\cite{gerberclas}
    There exists a constant $K_0$ such that for every $\epsilon$ small enough, \begin{equation}\label{parameq}
        n(\epsilon,\delta,\mathcal{P}_G(s,C_G)) \geq K_0 \frac{log(1/\delta)}{\epsilon^2}
    \end{equation}
\end{prop}
...
\begin{lemma}\label{klmixture}
    ...
    \end{equation}
\end{lemma}
\textcolor{red}{Proof in the appendix}\\
By combining \Cref{mixture} and \Cref{klmixture}, we obtain : 
\begin{equation*}
...
\begin{proof}[Proof of \Cref{samplecomplexitythm}]
